...
$(document).ready(function(){
	...
	$('form').on('submit', function(e){
	  e.preventDefault();
	  var query = $.trim($('.text').val().toString());
	  if(query == '') {
	    $('#temp').text('');
	    $('#desc').text('Nothing');
	  } else {
...
var getData = function(data){
	if('error' in data.data) {
	  ...
	  var temp = data.data.current_condition[0].temp_F,
	      colors = ['#4200ff', '#0648ff', '#0084ff', '#009dff', '#00b6ff','#00d6ff', '#00FF81', '#FFF254', '#FFAD00', '#FF7800','#ff6300', '#ff2d00', '#FF0300'];
	  $('#temp').text(temp);
	  $('#desc').text(data.data.current_condition[0].weatherDesc[0].value);
	  $('.text').val('').attr('placeholder',data.data.request[0].query);
	  if (temp < 0) {
